    ...
    def collect_metrics(self):
        ...
        for pool in pools:
            ...
            for vm in vms:
                if vm['type'] == 'qemu':
                    try:
                        vm_details = self.proxmox_client.get_vm_details(vm['node'], vm['vmid'])
                        vm_status = self.proxmox_client.get_vm_status(vm['node'], vm['vmid'])
                        ...
                        vm_metrics = {
                            'id': vm['vmid'],
                            'name': vm['name'],
                            'ip': vm_status.get('net0', {}).get('ip', ''),
                            'cpu_cores': int(vm_details.get('cores', 0)),
                            'memory_gb': int(vm_details.get('memory', 0)) / 1024,
                            'storage_gb': round(total_storage),
                            'backup_storage_gb': round(self.proxmox_client.get_backup_size(vm['node'], vm['vmid'])),
                            'os_type': vm_details.get('ostype', ''),
                            'tags': vm_details.get('tags', '').split(',') if vm_details.get('tags') else [],
                            'status': vm_status.get('status', '')
                        }
                        pool_metrics['virtual_machines'].append(vm_metrics)
                    except Exception as e:
                        print(f"Error fetching metrics for VM {vm['vmid']} in pool {pool['poolid']}: {str(e)}")
